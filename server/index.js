...
app.post('/api/story/save', function(req, res) {
    ...
    if (req.body.data.id) {
        var update = db.run("UPDATE story set data = ? WHERE id = ?", [JSON.stringify(req.body.data), req.body.data.id],
            function(err) {
                if (err) {
        ...
        var insert = db.run("INSERT INTO story(email,data) VALUES (?,?)", [req.body.email, JSON.stringify(req.body.data)],
            function(err) {
                if (err) {
                    console.log('error: ', err);
                    res.status(500).send({
...
app.post('/api/answer/create', function(req, res) {
    ...
    var insert = db.run("INSERT INTO answer(story_id,question_id,option_id) VALUES (?,?,?)", [req.body.story_id, req.body.question_id, req.body.option_id],
        function(err) {
            if (err) {
                console.log('error: ', err);
                res.status(500).send({
